...
exports.Crawler = function(options) {
    ...
    self.request = function(opts) {
        if (useCache(opts)) {
            ...
            var cacheData = unserialize(opts.cache.get(opts.uri));
            if (cacheData) {
                self.onContent(null, opts, cacheData, true);
                return;
            }
        }
    ...
    self.onContent = function (error, toQueue, response, fromCache) {
        ...
        console.log('Hit a cache, body size: ' + response.body.length + ' content:' + response.body.substring(0, 10));
        ...
        if (useCache(toQueue) && !fromCache) {
            if (toQueue.cache) {
                toQueue.cache.set(toQueue.uri, serialize(response, true));
